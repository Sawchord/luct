...
RUN uname -m
RUN wget -c https://github.com/wasm-bindgen/wasm-bindgen/releases/download/0.2.127/wasm-bindgen-0.2.127-$(uname -m)-unknown-linux-musl.tar.gz -O - | tar -xz
...
RUN ./wasm-bindgen-0.2.127-$(uname -m)-unknown-linux-musl/wasm-bindgen luct_extension.wasm --no-typescript --target web --out-dir luct/wasm
RUN tree luct
